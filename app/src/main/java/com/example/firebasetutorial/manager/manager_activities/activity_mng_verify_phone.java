...
public class activity_mng_verify_phone extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mng_verfiy_phone);

        progressDialog = new ProgressDialog(activity_mng_verify_phone.this);
...
//            Toast.makeText(activity_emp_verify_phone.this, s.toString()+"\nnew=old\n"+mResendToken.toString(), Toast.LENGTH_SHORT).show();
        ...
        public void onVerificationFailed(@NonNull FirebaseException e) { //get error
            progressDialog.dismiss();
            timeOutCode = true;
            Toast.makeText(activity_mng_verify_phone.this, "An error occurred while getting the code\n" + e.getMessage(), Toast.LENGTH_LONG).show();
        }
    };
    ...
    private void verifyCode(String codeByUser) { //compare between two codes
        try {
            ...
            Toast.makeText(activity_mng_verify_phone.this, "Wrong verification code entered\n" + e.getMessage(), Toast.LENGTH_LONG).show();
        }
    }

    private void openActivity(PhoneAuthCredential credential) {
        auth.getCurrentUser().updatePhoneNumber(credential).addOnCompleteListener(activity_mng_verify_phone.this, new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    ...
                    Toast.makeText(activity_mng_verify_phone.this, "An error occurred while updating the phone number due to:\n" + task.getException(), Toast.LENGTH_LONG).show();
                }
            }
    ...
    private void UpdatePhone(String phoneNumber) {
        ...
        auth.updateCurrentUser(auth.getCurrentUser()).addOnCompleteListener(activity_mng_verify_phone.this, new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    try {
                        db.getReference().child("User").child("Manager").child(uid).child("Phone").setValue(phoneNumber);
                        db.getReference().child("User").child("Manager").child(uid).child("DateTime_of_last_update").setValue(new SimpleDateFormat("dd/MM/yyyy - hh:mm:ss a", Locale.ENGLISH).format(Calendar.getInstance().getTime()).toString());
                        editor.putString("Phone", phoneNumber);
                        editor.apply();
                        Snackbar.make(findViewById(android.R.id.content), "Successfully update phone number", Snackbar.LENGTH_LONG).show();
                    } catch (Exception e) {
                        Toast.makeText(activity_mng_verify_phone.this, "Failed update phone number\n" + e.getMessage(), Toast.LENGTH_LONG).show();
                    }
                }
            }
        });
    ...
    private void showInfoAlert() {
        AlertDialog.Builder builder1 = new AlertDialog.Builder(activity_mng_verify_phone.this, R.style.Theme_MaterialComponents_Light_Dialog_Alert);
